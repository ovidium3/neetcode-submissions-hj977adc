class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        window = deque()
        # build first "window":
        # contains INDICES and not values
        for i in range(k):
        ...
        while r < len(nums):
            # popleft removes max val, if it is out of window range
            if window[0] < l:
                window.popleft()

            # pop all remaining vals if they are not high enough compared to current
            # one that we are about to add

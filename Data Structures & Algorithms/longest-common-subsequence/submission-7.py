class Solution: # O(len(text1) * len(text2)) time and space since bottom up table
    def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        ...
        # text1 = row, text2 = col
        # for some reason, need to initialize for i in range instead of *  since that creates
        # multiple references to the same list. this creates new individual lists each time
        ...
        for r in range(len(text1) - 1, -1, -1):
            for c in range(len(text2) - 1, -1, -1):
                ...
                if text1[r] == text2[c]:
                    dp[r][c] = 1 + dp[r + 1][c + 1] # need to take diagonal since that corresponds to both chars

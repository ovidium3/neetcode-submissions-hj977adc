        ...
        while pq:
            ...
            if currNode not in visited: # only process unvisited nodes
                ...
                for time, node in adjList[currNode]:
                    if node not in visited: # slight optimization to reduce heap operations and node reprocessing
                        heapq.heappush(pq, [time + runningTotal, node]) # add next node time to running total
